...
Forked from meertensm/amazon-mws. Expanded with basic SQS subscription methods. Fixed the GetLowestPricesOffersForASIN problem.
...
### Installation:
```bash
$ composer require blackdeer/amazon-mws
```
### Initiate the client
```php
...
// A method to send a test notification to an SQS url
$client->SendTestNotificationToDestination($destination);

// Register a SQS destination for subscription
$client->RegisterDestination($destination);

// Add a subscription buy passing a destination and subscription type (i.e. 'AnyOfferChanged')
$client->CreateSubscription($destination, $type);

// List all registered destination
$client->ListRegisteredDestinations();

// List all subscriptions
$client->ListSubscriptions();

// List inbound shipments
$client->ListInboundShipments();

// List inbound shipments with next token
$client->ListInboundShipmentsByNextToken($nextToken);

// List inbound shipment items
$client->ListInboundShipments($shipmentId);

// List inbound shipment items with next token
$client->ListInboundShipmentsByNextToken($nextToken);
```
